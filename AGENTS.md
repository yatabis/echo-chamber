...
- staged / unstaged / worktree / index を操作する場合は、`git status --short`、`git diff --name-status`、`git diff --cached --name-status` などで対象を切り分ける。特に `AM` / `MM` のような mixed state では、staged と unstaged を同じものとして扱わない
