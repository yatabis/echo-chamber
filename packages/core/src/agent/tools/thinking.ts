...
export const finishThinkingSessionRecordSchema = z.object({
  content: z
    .string()
    .min(1)
...
const finishThinkingParameters = {
  ...
  session_record: finishThinkingSessionRecordSchema.describe(
    '次回起動時の冒頭コンテキストに注入するためのセッション記録。メモリーに近い形式で、内容と感情を簡潔にまとめること。'
  ),
} satisfies z.ZodRawShape;

export const finishThinkingInputSchema = z.object(finishThinkingParameters);

export type FinishThinkingInput = z.infer<typeof finishThinkingInputSchema>;
export type FinishThinkingSessionRecord = z.infer<
  typeof finishThinkingSessionRecordSchema
>;
...
export const finishThinkingToolSpec = defineToolSpecification({
  ...
  parameters: finishThinkingParameters,
  outputSchema: createToolResultSchema({}),
});
