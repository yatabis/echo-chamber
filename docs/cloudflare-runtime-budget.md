...
`GET /:id/session-logs` は Dashboard Activity の材料になる event type と `session_id IS NOT NULL` を `LIMIT 200` より前に適用する。projection と query が同じ正の event type 集合を共有し、`idx_echo_events_archive_day_dashboard_activity_created` partial index から最大 200 candidate event の bounded read を維持する。既存 DO では、この partial index だけを維持する schema への初回 migration で、保持中の candidate event に対する index 構築と DDL に伴う rows read / written、storage 更新が発生する。定常時の API / DO request、event table row write、外部 API call は増えず、candidate event row にはこの partial index の保存・更新コストだけが加わる。
